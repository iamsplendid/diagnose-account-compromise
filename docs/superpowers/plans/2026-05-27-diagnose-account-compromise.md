# Diagnose-AccountCompromise Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a read-only PowerShell script that collects O365 account compromise indicators from Graph and Exchange Online, scores them, and renders a self-contained HTML triage report.

**Architecture:** Three phases — Collect (eight functions pulling from Graph and EXO), Analyze (`Invoke-IocAnalysis` scores findings into High/Medium/Low/Info), Render (`ConvertTo-HtmlReport` builds the HTML). All collectors wrap their calls in try/catch; a failed collector renders an error notice rather than aborting. Sign-in logs always pull 30 days so the investigation window can be compared against a baseline for new-country detection.

**Tech Stack:** PowerShell 5.1+, ExchangeOnlineManagement module, Microsoft.Graph module (scopes: AuditLog.Read.All, Directory.Read.All, UserAuthenticationMethod.Read.All)
...
| File | Purpose |
|---|---|
| `Diagnose-AccountCompromise.ps1` | Single-file script: all functions + execution block |
| `README.md` | Usage, prerequisites, example commands |

---

### Task 1: Create GitHub repo and initialize local repo

**Files:**
- Create: `README.md`
- Create: `Diagnose-AccountCompromise.ps1` (stub)

- [ ] **Step 1: Create the GitHub repo**

```bash
cd /mnt/c/github/diagnose-account-compromise
gh repo create iamsplendid/diagnose-account-compromise --public --description "Read-only O365 account compromise investigation and HTML report"
```

Expected: GitHub URL printed, e.g. `https://github.com/iamsplendid/diagnose-account-compromise`
...
- [ ] **Step 3: Create stub script file**

Create `Diagnose-AccountCompromise.ps1` with just a comment for now:
...
```bash
cd /mnt/c/github/diagnose-account-compromise
git remote add origin https://github.com/iamsplendid/diagnose-account-compromise.git
git add README.md Diagnose-AccountCompromise.ps1
git commit -m "Initial scaffold: README and stub script"
git push -u origin master
```

Expected: remote tracking set, both files visible on GitHub.

---

### Task 2: Script header, parameters, auto-update, and Write-Info helper

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (replace stub with full skeleton)

- [ ] **Step 1: Write the script skeleton**

Replace the entire content of `Diagnose-AccountCompromise.ps1` with:
...
.NOTES
    ...
    Graph scopes: AuditLog.Read.All, Directory.Read.All, UserAuthenticationMethod.Read.All
#>
[CmdletBinding()]
param(
...
powershell -NoProfile -Command "
    [ref]\$parseErrors = [ref]\$null
    \$null = [System.Management.Automation.Language.Parser]::ParseFile(
        (Resolve-Path '.\Diagnose-AccountCompromise.ps1').Path,
        [ref]\$null,
        \$parseErrors
    )
    if (\$parseErrors.Value) { \$parseErrors.Value | ForEach-Object { Write-Error \$_.Message }; exit 1 }
...
git commit -m "Add script skeleton, parameters, auto-update, and logging helpers"
```

---

### Task 3: Session validation (Assert-Sessions)

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append after Write-Fail helper)

- [ ] **Step 1: Append Assert-Sessions function**

Add after the `Write-Fail` function definition:
...
function Assert-Sessions {
    ...
    if (-not $graph) {
        throw "No active Graph session.`nRun: Connect-MgGraph -Scopes 'AuditLog.Read.All','Directory.Read.All','UserAuthenticationMethod.Read.All'"
    }

    $required = @('AuditLog.Read.All', 'Directory.Read.All', 'UserAuthenticationMethod.Read.All')
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add Assert-Sessions for EXO and Graph session validation"
```

---

### Task 4: Graph collectors — Get-UserProfile, Get-RiskyUserStatus, Get-AuthMethods

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append three collector functions)

All collectors return `[PSCustomObject]@{ Ok = $true; Data = ... }` on success and `[PSCustomObject]@{ Ok = $false; Error = '...'; Data = $null }` on failure.

- [ ] **Step 1: Append Get-UserProfile**
...
- [ ] **Step 2: Append Get-RiskyUserStatus**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add Graph collectors: Get-UserProfile, Get-RiskyUserStatus, Get-AuthMethods"
```

---

### Task 5: Graph collector — Get-SignInActivity

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append one collector function)

This collector always pulls the full 30-day retention window so the analysis phase can compare the investigation window against a prior baseline for new-country detection.

- [ ] **Step 1: Append Get-SignInActivity**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add Get-SignInActivity collector with 30-day pull and baseline/window split"
```

---

### Task 6: EXO collectors — Get-MailboxConfig and Get-InboxRules

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append two EXO collector functions)

- [ ] **Step 1: Append Get-MailboxConfig**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add EXO collectors: Get-MailboxConfig, Get-InboxRules"
```

---

### Task 7: EXO collectors — Get-MailboxAuditEvents and Get-TransportRuleMatches

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append two EXO collector functions)

- [ ] **Step 1: Append Get-MailboxAuditEvents**
...
- [ ] **Step 2: Append Get-TransportRuleMatches**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add EXO collectors: Get-MailboxAuditEvents, Get-TransportRuleMatches"
```

---

### Task 8: IOC analysis — Invoke-IocAnalysis

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append analysis function)

- [ ] **Step 1: Append Invoke-IocAnalysis**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add Invoke-IocAnalysis with all IOC scoring rules"
```

---

### Task 9: HTML report — ConvertTo-HtmlReport

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append renderer function)

- [ ] **Step 1: Append ConvertTo-HtmlReport**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add ConvertTo-HtmlReport with all nine cards and severity highlighting"
```

---

### Task 10: Main execution block

**Files:**
- Modify: `Diagnose-AccountCompromise.ps1` (append main execution block at end of file)

This is the entry point that wires all phases together.

- [ ] **Step 1: Append main execution block**
...
- [ ] **Step 2: Verify syntax (full script)**
...
```bash
git add Diagnose-AccountCompromise.ps1
git commit -m "Add main execution block: collect, analyze, render, write HTML"
```

---

### Task 11: Push to GitHub and smoke test

**Files:** (no changes — push and test only)
...
Expected: all commits on GitHub.

- [ ] **Step 2: Smoke test — syntax and help**

Confirm the script loads and shows its parameter help without a live session:

```powershell
powershell -NoProfile -Command "Get-Help .\Diagnose-AccountCompromise.ps1 -Full" 2>&1 | Select-String 'SYNOPSIS|PARAMETER|EXAMPLE' | Select-Object -First 10
```

Expected: SYNOPSIS, PARAMETER, and EXAMPLE sections visible.

- [ ] **Step 3: Smoke test — missing session error**

Confirm the script exits cleanly with an actionable message when no sessions exist:

```powershell
powershell -NoProfile -Command "
    & '.\Diagnose-AccountCompromise.ps1' -UserPrincipalName test@example.com -SkipUpdateCheck 2>&1
" | Select-Object -First 5
```

Expected: `[FAIL] No active Exchange Online session.` (or Graph equivalent) — not a raw exception stacktrace.

- [ ] **Step 4: Verify repo on GitHub**

```bash
gh repo view iamsplendid/diagnose-account-compromise
```

Expected: repo description, README preview, correct visibility.

- [ ] **Step 5: Save memory about this project**

Save a project memory at `/home/spierce/.claude/projects/-mnt-c-github-diagnose-mailbox/memory/project_diagnose_account_compromise.md` with:

```markdown
---
name: project-diagnose-account-compromise
description: New script and repo for O365 account compromise investigation with HTML report
metadata:
  type: project
---

Repo: /mnt/c/github/diagnose-account-compromise on GitHub as iamsplendid/diagnose-account-compromise

Script: Diagnose-AccountCompromise.ps1 v1.0.0

Architecture: Collect (8 functions) -> Analyze (Invoke-IocAnalysis) -> Render (ConvertTo-HtmlReport)

Data sources: Graph sign-in logs, Identity Protection, auth methods; EXO mailbox config, inbox rules, audit log, transport rules.

**Why:** New standalone script per the pattern (each script = its own repo).
**How to apply:** Treat as independent from diagnose-mailbox repo. Update URL in ScriptUpdateUrl if repo ever moves.
```

And add an entry to `MEMORY.md`.
